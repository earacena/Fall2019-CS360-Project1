...
//print assembly code based on the parsed source
void Parser::print_assembly()
{
    std::cout<<"-----Now translating-----"<<std::endl;
    Statements* current = nullptr;
    for(Function function : parsed_source)
    {
        current = &function.instructions;
        while(current != nullptr)
        {
            ...
            {
                translator.translate_logic(current->lo_instr);
            }
            else if(current->type == "declaration")
            {
                translator.translate_declaration(current->dec_instr);
            }
            current = current->next;
        }
        translator.print();
